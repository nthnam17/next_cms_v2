...
export default function NextDialogForm({
  ...
  return (
    <Dialog open={open} onOpenChange={onToggle}>
      ...
      <DialogContent
        className={cn("px-3 py-4 sm:rounded-3xl bg-[#f4f4f4]", className)}
      >
            ...
            >
              {loading ? (
                <div className="relative w-5 h-5 animate-spin">
                  <NextImg src="/assets/icons/loading-btn.svg" alt="Capi" />
                </div>
              ) : (
                <React.Fragment>
                  <i>
                  ...
                  <p className="ml-2 text-sm uppercase text-white font-light">
                    {btnTitle ? btnTitle : isCreate ? "Tạo mới" : "Cập nhật"}
                  </p>
                </React.Fragment>
              )}
            </Button>
